...
#include "rom_spectrum.h"
unsigned char RAM[48 * 1024];
Z80_STATE state;
...
{                                                                       \
        uint16_t a = ((address) & 0xffff);                              \
        if (a < 0x4000) { /* error if address overlap RAM-ROM ? */      \
          (x) = pgm_read_byte(ROM + a) | (pgm_read_byte(ROM + a + 1)) << 8;  \
        } else {                                                        \
          (x) = RAM[a - 0x4000] | (RAM[a - 0x4000 + 1]) << 8;           \
        }                                                               \
}
...
{                                                                       \
        (x) = input_byte((state->registers.byte[Z80_B]) << 8 | (port));                              \
        /*SystemCall(state);*/                                          \
}
